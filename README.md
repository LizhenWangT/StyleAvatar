...
>Modify the file `info.json` before you run your own model: Change the input video and save dir path to your path. `skip_frames` is used to skip the first several frames without portrait regions. **Crop_size should be 1024 for styleunet and 1576 for full styleavatar.** For cross person testing, you should change the `change_id` to `true` and give the input id_path.
