...
>For python code: see FaceVerse v3 for more details: https://github.com/LizhenWangT/FaceVerse/tree/main/faceversev3_jittor. When testing, as shown in Line 129-134 of `FaceVerse/faceversev3_jittor/tracking_offline_cuda.py`, you need to change the first 150 dims (shape parameters of another person) of the jittor tensor `coeffs` in Line 123 to the values (shape parameters of the source actor) in `id.txt` generated by `FaceVerse/faceversev3_jittor/tracking_offline_cuda.py`. If you are testing cross person re-enactment cases and **only if the first frame is neutral expression**, add the first expression in `exp.txt` to the exp param (id and exp are coupling with each other, so this operation can improve the cross person results).
